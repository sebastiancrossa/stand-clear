...
public struct StationCatalog: Sendable {
    ...
    /// The `count` nearest parent stations that serve any of `routes`, sorted nearest-first.
    /// Stations in the same transfer complex collapse to the nearest member so a complex
    /// never appears twice on the board.
    public func nearestStations(
        to location: CLLocation,
        count: Int,
        servingAnyOf routes: Set<String>
    ) -> [(station: Station, distance: CLLocationDistance)] {
        let normalizedRoutes = Set(routes.map(RouteID.normalized))
        guard count > 0, !normalizedRoutes.isEmpty else { return [] }

        var bestByComplex: [String: (station: Station, distance: CLLocationDistance)] = [:]
        for station in stations {
            let served = self.routes(serving: station.id)
            guard !served.isDisjoint(with: normalizedRoutes) else { continue }

            let distance = station.location.distance(from: location)
            let complexKey = relatedStations(to: station.id).sorted().first ?? station.id
            if let existing = bestByComplex[complexKey], existing.distance <= distance {
                continue
            }
            bestByComplex[complexKey] = (station, distance)
        }

        return bestByComplex.values
