...
struct LiveMapWindowView: View {
    ...
    @State private var pickerSnapshots: [TrainRenderSnapshot] = []
    ...
    var body: some View {
        ...
        .onReceive(model.$mapMotionPlans) { plans in
            clusterSnapshots = []
            refreshPickerSnapshots(from: plans)
            reconcileSelection()
        }
        .onChange(of: session.routeSelection) { _, _ in
            refreshPickerSnapshots(from: model.mapMotionPlans)
        }
    }
    ...
    private func liveMap(_ geometry: TrackGeometryCatalog) -> some View {
        let selectedSnapshot = session.selectedTrainID.flatMap { selectedID in
            model.mapMotionPlans.first { $0.id == selectedID }?.render(at: model.now)
        }
        return ZStack {
            LiveSubwayMap(
                geometry: geometry,
    ...
    private func refreshPickerSnapshots(from plans: [TrainMotionPlan]) {
        pickerSnapshots = plans
    ...
    private func reconcileSelection() {
        let liveTrainIDs = Set(model.mapMotionPlans.lazy.filter {
            $0.dataHealth(at: model.now) != .expired
        }.map(\.id))
