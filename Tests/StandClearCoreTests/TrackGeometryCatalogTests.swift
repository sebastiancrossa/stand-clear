...
final class TrackGeometryCatalogTests: XCTestCase {
    ...
    func testBinaryRoundTripPreservesRouteMetadataAndPointCounts() throws {
        let bundled = try TrackGeometryCatalog.bundled()
        let encoded = try GeometryBinaryCodec.encode(bundled.resource)
        let restored = try TrackGeometryCatalog(data: encoded)

        XCTAssertEqual(restored.resource.version, SubwayGeometryResource.currentVersion)
        XCTAssertEqual(restored.resource.routes.count, bundled.resource.routes.count)
        XCTAssertEqual(restored.resource.paths.count, bundled.resource.paths.count)
        XCTAssertEqual(restored.route("A")?.colorHex, bundled.route("A")?.colorHex)
        XCTAssertEqual(
            restored.resource.paths.map(\.points.count),
            bundled.resource.paths.map(\.points.count)
        )
    }
}
