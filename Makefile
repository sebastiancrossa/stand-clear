...
refresh-stops:
	...
	awk -F, 'FNR == NR {if (FNR > 1) parent[$$1] = ($$6 == "" ? $$1 : $$6); next} ($$1 in parent) {print parent[$$1] "," $$2}' Sources/SubwayBarCore/Resources/stops.txt .build/mta-static/stop_routes.csv | sort -u | awk 'BEGIN {print "station_id,route_id"} {print}' > .build/mta-static/station_routes.csv
	test "$$(wc -l < .build/mta-static/station_routes.csv)" -gt 1
	cp .build/mta-static/station_routes.csv Sources/SubwayBarCore/Resources/station_routes.csv
